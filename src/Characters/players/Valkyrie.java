...
public class Valkyrie extends BasePlayer{
    public Valkyrie(Images image, Pane layer, double x, double y, double r, double dx, double dy, double dr, double health, double damage, double range, Teams teamType, List<BaseTower> towers, double delay, List<BaseBullet> bullets, List<BaseCharacter> opponents) {
        super(image, layer, x, y, r, dx, dy, dr, health, damage, range, teamType, towers, delay, bullets, opponents);
        imageView.setImage(image.valkyrieImage);
        imageView.setFitHeight(22.5);
    ...
    public boolean hitTarget() {
        ...
        if(inRange(target)){
            if(nextHit==null){
                ...
                for(BaseCharacter character: opponents){
